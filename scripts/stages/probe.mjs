 ...
 *   - the page answers 200, and is the page it should be;
...
const domain = process.argv[3] ?? 'dev.localhost';
const STAGES = [
  { id: 'sitemap', n: 1 },
  { id: 'wireframe', n: 2 },
  { id: 'skeleton', n: 3 },
...
await expectPage('hub by path', 'localhost', '/stages', 'in five fidelities');
...
for (const s of STAGES) {
  ...
  await expectPage(`${s.id} by subdomain`, `${s.id}.${domain}`, '/rsvp', `Stage ${s.n}</title>`);
...
console.log(`stage hosting probe: all ${checks} checks passed (hub, 4 stages × subdomain + path, their assets, stage 4 fonts, the app's own home untouched).`);
