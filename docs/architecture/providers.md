...
| `flights` | `search(req) -> LiveSnapshot<FlightResult[]>`, `deepLink(req)` (the Skyscanner builder throws `RangeError` unless codes are 3-letter IATA and dates `YYYY-MM-DD`: validate input with zod first) | `MockFlights`: fixtures into ORD/MDW labelled "Mock Airways"; Skyscanner deep link | `DeepLinkOnlyFlights` (`FLIGHTS_PROVIDER=deep-link`): search unavailable, links work. Live (Duffel) is the travel swarm's | `FLIGHTS_PROVIDER`, `DUFFEL_API_KEY` (reserved) | mock |
