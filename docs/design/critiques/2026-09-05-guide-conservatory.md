# Design critique — `/share-an-adventure` (Share an Adventure) — Conservatory — 2026-09-05
...
**Weighted: 6.2 / 10 — REDESIGN (navigation).** Ship threshold: every axis ≥ 7 with Usability ≥ 8.
No page on this level met it before the fixes; none of these numbers has been re-scored.

## What changed since that review

- **BL-3** — same outline fix and jump list; this design's Menu tag also occluded live text at **15 scroll offsets** on this page, and it is now reserved for.
- **BL-2** — 89.25 px reserved against 60 px of chrome.
- **SF-7** — provider slugs are gone.
- The jump list's kraft tags drop the small-caps tracking, because a jump label is a phrase.
...
- `IMPECCABLE_BROWSER=… npx impeccable detect "http://localhost:3105/share-an-adventure?theme=conservatory"`: **exit 0** (all 20 URLs including Home)
