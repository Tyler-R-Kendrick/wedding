# Design critique — `/explore-caa` (Explore CAA) — Conservatory — 2026-09-05
...
- **BL-4** — same: the jar label's `<h3>` is the name, the link is its own control naming the host.
- **BL-1** — 4 ticket references removed.
- **BL-2** — the Menu tag occluded text at 5 scroll offsets here; it is now reserved for.
- **SF-10** — the icon no longer orphans.
...
- `IMPECCABLE_BROWSER=… npx impeccable detect "http://localhost:3105/explore-caa?theme=conservatory"`: **exit 0** (all 20 URLs including Home)
