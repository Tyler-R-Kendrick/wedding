# Design critique — `/the-wedding` (The Wedding) — Conservatory — 2026-09-05
...
- **BL-2** — the Menu tag covered the dress-code hint **at scroll 0**; `main` now reserves 89.25 px against 60 px of chrome.
- **BL-1** — the same 11 ticket references; **0** remain.
- **BL-6** — measured at 390: “What to wear” at **619→649** with its answer starting at **731**, inside the fold.
- Still open (**SF-9**): the programme's hints run past this design's 55–72 character measure at 1440 now that the vine spans the sheet.
...
- `IMPECCABLE_BROWSER=… npx impeccable detect "http://localhost:3105/the-wedding?theme=conservatory"`: **exit 0** (all 20 URLs including Home)
