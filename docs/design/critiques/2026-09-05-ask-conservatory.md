# Design critique — `/ask-us` (Ask Us) — Conservatory — 2026-09-05
...
**Weighted: 7.4 / 10 — FIX FIRST.** Ship threshold: every axis ≥ 7 with Usability ≥ 8.
No page on this level met it before the fixes; none of these numbers has been re-scored.

## What changed since that review

- **BL-1** — the same 7 ticket references; **0** remain.
- **SF-4** — the eight FAQ links were 27 px; they are now 44 px.
- **BL-2** — the Menu tag is reserved for.
- The review's strongest page in either design (7.4). Its search, labels and live region were already on contract and are unchanged.
...
- `IMPECCABLE_BROWSER=… npx impeccable detect "http://localhost:3105/ask-us?theme=conservatory"`: **exit 0** (all 20 URLs including Home)
