...
and a note strip naming the route, the capability and what to check. The
wedding design tokens are inlined **verbatim** from
`src/themes/<id>/theme.css` at build time, so a lab cannot drift from the
shipped kits — regenerate after any token change.
...
`build.mjs` inlines `src/_shell.css`, `src/_shell.js` and both kits' token
blocks into each fragment, then writes a single self-contained file. The
generated `--art-*` URLs are dropped: they point at `/assets/art/*` on the
real origin, which a standalone page cannot fetch.
...
```
src/_shell.css     workbench chrome + site primitives built on theme tokens
