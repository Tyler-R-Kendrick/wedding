...
describe('upload engine', () => {
  ...
  it('uploads a batch: single PUT, multipart parts in order, duplicates and rejections reported per file', async () => {
    ...
    // Asset ids are asserted against each job's OWN upload session, never against a literal `U1`.
    // `pump()` runs two jobs at a time and each calls `api.create` separately, so which file the
    // server hands `U1` to is decided by which request arrives first — a race, and the reason this
    // test failed roughly once in ten runs with "expected 'ASSET_U2' to be 'ASSET_U1'". The
    // guarantee is that a job ends up holding the asset minted for its own session; the order two
    // concurrent uploads reach the server is not a guarantee and never was.
    expect(a!.state).toBe('processing');
    expect(a!.uploadId).toBeDefined();
    expect(a!.assetId).toBe(`ASSET_${a!.uploadId}`);
    expect(a!.progress).toBe(1);
    expect(b!.state).toBe('processing');
    expect(Object.keys(b!.parts).map(Number)).toEqual([1, 2, 3]);
    expect(server.uploads.get(b!.uploadId!)!.completed).toBe(`ASSET_${b!.uploadId}`);
    ...
    up.markProcessed(new Set([a!.assetId!]), new Map([[b!.assetId!, 'no good']]));
