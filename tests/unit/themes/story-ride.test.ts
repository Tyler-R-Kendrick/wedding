...
import { stepFactor, trainPosition } from '@/themes/botanical-deco/kit/StoryRide';
...
describe('Our Story ride', () => {
  it('stops the train at every station before it moves on', () => {
    // The first half of each stop's scroll is spent standing at the platform.
    expect(trainPosition(0, 5)).toBe(0);
    expect(trainPosition(0.49, 5)).toBe(1 - 1);
    expect(trainPosition(1.2, 5)).toBe(1);
    // Then it accelerates away and brakes into the next one: slow, fast, slow.
    const early = trainPosition(1.6, 5) - 1;
    const middle = trainPosition(1.75, 5) - 1;
  ...
  it('never runs off either end of the line', () => {
    ...
    expect(trainPosition(4.4, 5)).toBe(4);
