...
describe('execution rules', () => {
  ...
  it('carries the descriptor output cap, defaulting to the pipeline default', () => {
    // Read from the descriptor, not written as a literal. This asserted `2_000`, which was
    // site_status's cap on swarm K's level-03 base; a later level tuned it to 4_000 and the test
    // failed for a reason that has nothing to do with the derivation it exists to check. The
    // guarantee is "whatever the descriptor says, the rules carry it".
    expect(siteStatus.maxOutputChars).toBeDefined();
    expect(deriveExecutionRules(siteStatus).maxOutputChars).toBe(siteStatus.maxOutputChars);
