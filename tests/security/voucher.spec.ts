...
const asGuest = (name: 'A1' | 'A2' | 'B1') => json(principalHeaders(name));
...
test.describe('voucher security', () => {
  ...
  test('a ride credit is only ever visible to the guest who owns it', async ({ request }) => {
    ...
    const owner = IDS.A1;
    const household = IDS.householdA;
    ...
    const draft = await request.post('/api/capabilities/draft_my_transportation_claim', { headers: asGuest('A1'), data: { input: { entitlementId } } });
    expect(draft.status()).toBe(200);
    const token = (await draft.json()).confirmation.token as string;
    const missingKey = await request.post('/api/capabilities/claim_my_transportation_benefit', { headers: asGuest('A1'), data: { input: { entitlementId }, confirmationToken: token } });
    expect(missingKey.status()).toBe(422);
    const claim = await request.post('/api/capabilities/claim_my_transportation_benefit', { headers: asGuest('A1'), data: { input: { entitlementId }, idempotencyKey: `sec-claim-${stamp}`, confirmationToken: token } });
    ...
    const mine = await request.post('/api/capabilities/get_my_transportation_options', { headers: asGuest('A1'), data: { input: {} } });
    ...
    const others = [
      { label: 'a sibling in the same household', headers: asGuest('A2'), ownsIt: false },
      { label: 'a guest in another household', headers: asGuest('B1'), ownsIt: false },
    ...
    const csrf = await request.post('/api/capabilities/claim_my_transportation_benefit', { headers: { ...principalHeaders('A1'), origin: 'https://evil.example', 'content-type': 'application/json' }, data: { input: { entitlementId }, idempotencyKey: `sec-csrf-${stamp}`, confirmationToken: token } });
