...
import { installTestPrincipalResolver, TEST_AUTH_HEADER, TEST_PRINCIPAL_HEADER } from '@/domain/testing/testPrincipal';
...
const GUEST_A = FX.guestA1;

const guestA: GuestPrincipal = { kind: 'guest', authIdentityId: 'auth-a' as AuthIdentityId, guestId: GUEST_A as GuestId, householdId: FX.householdA as HouseholdId, actsFor: [GUEST_A as GuestId], entitlements: new Set(['upload_media', 'view_private_media', 'use_face_matching']), authenticatedAt: new Date().toISOString(), sessionId: 'sa' };
...
async function post<T>(action: string, body: Record<string, unknown>, principal: 'guestA' | 'guestB' | 'anon' = 'guestA', extraHeaders: Record<string, string> = {}) {
  const specs: Record<string, unknown> = {
    guestA: { kind: 'guest', guestId: GUEST_A, householdId: FX.householdA, entitlements: ['upload_media', 'view_private_media', 'use_face_matching'] },
    guestB: { kind: 'guest', guestId: FX.guestB1, householdId: FX.householdB, entitlements: ['upload_media', 'view_private_media', 'use_face_matching'] },
  };
  ...
  if (principal !== 'anon') {
    headers[TEST_PRINCIPAL_HEADER] = JSON.stringify(specs[principal]);
    // `x-test-auth`, from identity's resolver. Swarm I inherited swarm H's own injector and its
    // `x-test-auth-secret` header; level 10 deleted that file as the fourth swarm-local principal
    // resolver, so this suite now speaks to the one resolver the site actually installs.
    headers[TEST_AUTH_HEADER] = TEST_AUTH_SECRET;
  }
...
/** A grant row in the vault's append-only ledger, returning its id so an enrolment can name it. */
async function insertGrant(db: Awaited<ReturnType<typeof getDb>>, guestId: string, householdId: string, over: { policyVersion?: string; text?: string; grantedAt?: Date } = {}): Promise<string> {
  const id = newId();
  const at = over.grantedAt ?? new Date();
  await db.insert(biometricConsents).values({
    id, guestId, householdId, entry: 'grant', grantId: null,
    policyVersion: over.policyVersion ?? '2026-01-01.v1',
    textHash: 'a'.repeat(64), text: over.text ?? 'the wording shown', purpose: 'p', term: 't', retention: 'r',
    providerDisclosure: 'd', scope: 'self_match', adultAttested: true, ipHash: null, surface: 'ui',
    requestId: newId(), grantedAt: at, revokedAt: null, createdAt: at,
  });
  return id;
}

/** The guest's standing grant row — see the note at each call site. */
async function grantIdFor(db: Awaited<ReturnType<typeof getDb>>, guestId: string): Promise<string> {
  const rows = await db.select().from(biometricConsents).where(eq(biometricConsents.guestId, guestId));
  const grant = rows.find((r) => r.entry === 'grant');
  if (!grant) throw new Error(`no grant row for ${guestId}; the fixture expects one`);
  return grant.id;
}

describe('biometric subsystem (gated off by default)', () => {
  beforeAll(async () => {
    // The corpus these cases match against is real `media_assets`, which reference `guests` and
    // `households` as of level 10. The vault's own tables deliberately do NOT (see the note in
    // src/db/schema/biometrics.ts), but the photographs it matches over certainly do.
    await seedSwarmE();
    ...
    corpus = await placeCorpus(db, storage, new FakeMediaAi(), [
      ...
      { ref: 'theirs', collectionSlug: 'guest-uploads', ownerGuestId: FX.guestB1, ownerHouseholdId: FX.householdB, caption: 'Someone else' },
      { ref: 'hidden-from-me', collectionSlug: 'guest-uploads', ownerGuestId: FX.guestB1, ownerHouseholdId: FX.householdB, visibility: 'private', caption: 'Not visible to me' },
  ...
  describe('what a deletion has to reach', () => {
    ...
    it('never leaves the biometric result in the public idempotency table', async () => {
      ...
      expect(await db.select().from(idempotencyKeys).where(eq(idempotencyKeys.scope, `find_photos_of_me:${guestScopeKey(FX.guestB1)}`))).toHaveLength(0);
    ...
    it('purges a cached response written before the opt-out existed, and proves the index is empty', async () => {
      ...
      await db.insert(idempotencyKeys).values({
        scope: `find_photos_of_me:${guestScopeKey(FX.guestB1)}`,
      ...
      const nonces = await db.select().from(idempotencyKeys).where(eq(idempotencyKeys.scope, `confirm:grant_biometric_consent:${guestScopeKey(FX.guestB1)}`));
  ...
  describe('withdrawal, deletion and retention', () => {
    ...
    it('withdrawing again after a failed deletion still queues one, rather than doing nothing', async () => {
      ...
      // The grant this enrolment belonged to, not a filler id. `identity_refs.consent_id` is a real
      // key within the vault as of this level, and the scenario being simulated — a reference that
      // survived a failed deletion — never involved a missing consent: the ledger is append-only, so
      // a revocation ADDS a row and the grant it withdraws is still there. The constraint made the
      // fixture honest rather than making the case impossible.
      await db.insert(biometricIdentityRefs).values({
        id: newId(), guestId: GUEST_A, consentId: await grantIdFor(db, GUEST_A), providerName: 'mock', subjectId: GUEST_A,
    ...
    it('the retention sweep asks for deletion of anything older than the window', async () => {
      const db = await getDb();
      await db.delete(biometricDeletions);
      // The grant this enrolment belongs to. It was referenced by a literal id that was never
      // inserted; `identity_refs.consent_id` is a real key within the vault now, and an enrolment
      // that names no consent is precisely the row that must not exist.
      const oldConsentId = await insertGrant(db, 'GUESTOLD', 'HOUSEOLD', { grantedAt: new Date(Date.now() - 400 * 86_400_000) });
      await db.insert(biometricIdentityRefs).values({
        id: newId(),
        guestId: 'GUESTOLD',
    ...
    it('sweeps a template whose consent is superseded, not only one that has aged out', async () => {
      ...
      await db.delete(biometricDeletions);
      await db.delete(biometricIdentityRefs);
      // ...whose grant was for an older version of the consent copy. Inserted FIRST so the
      // enrolment below can reference it: this fixture previously wrote the consent with `newId()`
      // while the enrolment named a literal `CONSENTSUPER`, so the two never referred to each other
      // even within the test's own data. The key made that visible.
      const supersededConsentId = await insertGrant(db, 'GUESTSUPER', 'HOUSESUPER', {
  ...
  describe('the consent ledger cannot hold two open grants', () => {
    ...
    it('two tabs racing through the public endpoint also produce exactly one grant', async () => {
      ...
      const grants = (await db.select().from(biometricConsents).where(eq(biometricConsents.guestId, FX.guestB1))).filter((r) => r.entry === 'grant');
      expect(grants).toHaveLength(1);
    });
