...
const guestB: GuestPrincipal = { ...guestA, authIdentityId: 'auth-b' as AuthIdentityId, guestId: 'GUESTB' as GuestId, householdId: 'HOUSEB' as HouseholdId, actsFor: ['GUESTB' as GuestId], entitlements: new Set(['upload_media', 'view_private_media']), sessionId: 'sb' };
...
describe('media pipeline (PGlite + local-fs storage)', () => {
  ...
  it('single upload: PUT to the signed URL, complete, process, derive; the served derivative has no GPS', async () => {
    ...
    expect(asset.originalKey).toBe(`originals/guest/GUESTA/${firstAssetId}.jpg`);
  ...
  it('video: MP4 is structurally validated, location atoms are stripped from the served copy, a poster exists, playback is signed', async () => {
    ...
    expect(asset.originalKey).toBe(`originals/guest/GUESTA/${assetId}.mp4`);
  ...
  it('guests delete only their own uploads, completely, leaving an archive manifest', async () => {
    ...
    expect(manifest.ok && manifest.value && JSON.parse(new TextDecoder().decode(manifest.value.body))).toMatchObject({ assetId, mode: 'hard', deletedBy: { kind: 'guest', guestId: 'GUESTA' } });
