...
export const GUEST_A = FX.guestA1;
export const GUEST_B = FX.guestB1;

export const guestA: GuestPrincipal = {
  kind: 'guest', authIdentityId: 'auth-a' as AuthIdentityId, guestId: GUEST_A as GuestId, householdId: FX.householdA as HouseholdId,
...
export const guestB: GuestPrincipal = { ...guestA, authIdentityId: 'auth-b' as AuthIdentityId, guestId: GUEST_B as GuestId, householdId: FX.householdB as HouseholdId, actsFor: [GUEST_B as GuestId], sessionId: 'sb' };
...
export async function buildRig(): Promise<Rig> {
  ...
  // Same reason as tests/integration/media-ai.test.ts: the corpus owns its assets as a real guest.
  const { seedSwarmE } = await import('../helpers/swarm-e');
  await seedSwarmE();
  ...
  const corpus = await placeCorpus(db, storage, new FakeMediaAi(), [
    ...
    { ref: 'theirs', collectionSlug: 'guest-uploads', ownerGuestId: GUEST_B, ownerHouseholdId: FX.householdB, caption: 'Someone else' },
    { ref: 'household-b', collectionSlug: 'guest-uploads', ownerGuestId: GUEST_B, ownerHouseholdId: FX.householdB, visibility: 'household', caption: 'Bravo household only supper' },
    { ref: 'hidden-from-me', collectionSlug: 'guest-uploads', ownerGuestId: GUEST_B, ownerHouseholdId: FX.householdB, visibility: 'private', caption: 'Secret zebra manuscript' },
    { ref: 'unpublished', collectionSlug: 'guest-uploads', ownerGuestId: GUEST_B, ownerHouseholdId: FX.householdB, status: 'private', caption: 'Unpublished walrus rehearsal' },
    { ref: 'quarantined', collectionSlug: 'guest-uploads', ownerGuestId: GUEST_B, ownerHouseholdId: FX.householdB, status: 'quarantined', caption: 'Quarantined ocelot banner' },
...
export async function post<T>(action: string, body: Record<string, unknown>, principal: 'guestA' | 'guestB' | 'anon' = 'guestA', extraHeaders: Record<string, string> = {}) {
  const specs: Record<string, unknown> = {
    ...
    guestB: { kind: 'guest', guestId: GUEST_B, householdId: FX.householdB, entitlements: ['upload_media', 'view_private_media', 'use_face_matching'] },
  };
