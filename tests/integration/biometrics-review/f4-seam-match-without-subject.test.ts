...
import { biometricConsents, biometricIdentityRefs } from '@/db/schema/biometrics';
...
describe('F4: the seam identifies subjects who have withdrawn (or never gave) consent', () => {
  ...
  it('a match with no subjectId runs after every consent in the system is gone', async () => {
    ...
    // Enrolments first — `identity_refs.consent_id` is a real key within the vault as of the level
    // that integrated this branch, so the ledger cannot be emptied out from under an enrolment that
    // an entry in it authorised. That is the constraint working; the assertion below is unchanged.
