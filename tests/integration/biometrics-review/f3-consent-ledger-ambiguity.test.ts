...
describe('F3: concurrent grants leave the consent ledger ambiguous', () => {
  ...
  it('two concurrent grants for one guest both succeed, and only one can ever be withdrawn', async () => {
    ...
    const base = { guestId: GUEST_A, householdId: FX.householdA, policyVersion: CONSENT_POLICY_VERSION, textHash: CONSENT_TEXT_HASH, adultAttested: true, ipHash: 'h', surface: 'ui' };
  ...
  it('the same race is reachable through the public consent endpoint (two tabs)', async () => {
    ...
    const rows = (await db.select().from(biometricConsents).where(eq(biometricConsents.guestId, FX.guestB1))).filter((r) => r.entry === 'grant');
    console.info('[F3-http] grant1=%s grant2=%s ledgerGrants=%d state=%s', g1.ok, g2.ok, rows.length, (await getConsentState(db, FX.guestB1)).status);
