...
export async function placeCorpus(db: Db, storage: StorageProvider, mediaAi: FakeMediaAi, items: readonly CorpusItem[], now: Date): Promise<Map<string, PlacedItem>> {
  ...
  for (const item of items) {
    ...
    await db.insert(mediaAssets).values({
      ...
      // Seeded fixture ids, not the synthetic `GUESTA`/`HOUSEA` these replace. `media_assets`
      // references `guests` and `households` for real as of level 10, so every caller of this
      // helper — the media-ai integration suite, the cluster and eligibility units, and the
      // adversarial review probes — failed on the constraint before reaching what it was testing.
      // Fourth level running that an ownership column shipped as bare text and the database, not a
      // test, was what noticed.
      ownerGuestId: item.ownerGuestId ?? (item.source === 'professional' ? null : FX.guestA1),
      ownerHouseholdId: item.ownerHouseholdId ?? (item.source === 'professional' ? null : FX.householdA),
      ...
      originalKey: `originals/guest/${item.ownerGuestId ?? FX.guestA1}/${assetId}.jpg`,
      quarantineKey: null,
