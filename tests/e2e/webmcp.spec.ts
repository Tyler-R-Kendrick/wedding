...
import { customPrincipalHeaders, IDS, principalHeaders } from './helpers/principal';
 ...
 *
 * A signed-in guest comes from the CANONICAL test-only principal injector
 * (src/domain/testing/testPrincipal.ts), honoured solely under NODE_ENV=test with a
 * constant-time-compared TEST_AUTH_SECRET. Swarm K wrote this against its own local injector and
 * three canned kinds; that was the sixth swarm-local principal resolver and was deleted at
 * integration, so the kinds below are expressed as real specs over the shared helper instead. This
 * spec runs in the TEST_SERVER arrangement, which always sets the secret.
 */
...
/**
 * K's three canned kinds, over the shared injector. `guest` is deliberately STALE — K used it for
 * the step-up cases, and a principal authenticated long ago is what makes `stepUp: true` bite —
 * while `guest-fresh` authenticated just now. Expressing them as specs rather than as magic strings
 * is what lets one injector serve every level.
 */
const STALE = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
const as = (kind: Kind): Record<string, string> => {
  if (kind === 'admin') return principalHeaders('admin');
  if (kind === 'guest-fresh') return principalHeaders('A1');
  return customPrincipalHeaders({ kind: 'guest', guestId: IDS.A1, householdId: IDS.householdA, actsFor: [IDS.A1], authenticatedAt: STALE });
};
