...
import { EVAL_PRINCIPALS, type EvalPrincipalName } from './principals';

export { EVAL_PRINCIPALS, EVAL_TABLES, type EvalPrincipalName } from './principals';
...
export async function runEvalCase(testCase: EvalCase): Promise<CaseReport> {
  ...
  // A refusal has no sources — its links ARE how it points the guest at the page that can help, so
  // they count here. Scoring only `sources` marked "we cannot do that, but /rsvp can" as uncited.
  const offered = [...result.sources.map((s) => s.url), ...(result.refusal?.links ?? []).map((l) => l.href)].filter(Boolean);
  for (const url of expected.citesUrl ?? []) {
    if (!offered.includes(url)) failures.push(`missing citation ${url}; got ${offered.join(', ') || '(none)'}`);
  }
