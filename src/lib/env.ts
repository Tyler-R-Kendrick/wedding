...
const serverSchema = z.object({
  ...
  /** Test-only principal injection (`x-test-principal` header). Honored only when NODE_ENV=test AND this secret matches. */
  ...
  /** Media pipeline (Swarm H). Caps are per file; parts must be >= 5 MiB on S3/R2 (local-fs accepts smaller for tests). */
