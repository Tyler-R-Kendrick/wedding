...
export const COUNSEL_REVIEW_REF = z
  .string()
  .trim()
  // 8, not 12: the guidance below offers `ADR-0006 §7` as an acceptable answer and the message on
  // the regex repeats it, but that string is 11 characters, so the validator rejected its own
  // documented example. The regex is what actually enforces a followable shape.
  .min(8, 'Reference the review itself: a URL, an ADR section, or a dated memo reference.')
  .max(200)
