...
// The same gate the canonical resolver uses (NODE_ENV=test + a >=16-char TEST_AUTH_SECRET),
// read from the one module that owns it rather than from a local copy.
import { isTestPrincipalEnabled } from '@/domain/testing/testPrincipal';
...
if (isTestPrincipalEnabled()) fixtureRegistry.registerAll(WEBMCP_TEST_CAPABILITIES);
