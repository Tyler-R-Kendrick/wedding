...
export async function handleManifest(request: Request): Promise<Response> {
  ...
  // Shared with the concierge's tool list since level 15 (src/capabilities/readiness.ts):
  // two derived lists that disagreed about readiness is exactly the drift this prevents.
  const unreadyFlags = await unreadyGatedFlags(flags, (flag) => isReady(flag, db));
