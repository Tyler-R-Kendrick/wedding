...
export function planRoute(question: string, available: readonly RouterTool[], everything: readonly AnyCapability[], maxCalls: number): RoutePlan {
  ...
  if (personal) {
    // Ranked, not registry order. `add` stops at maxCalls, so an unranked walk let whichever
    // personal capability happened to be registered first take the budget: on the full registry
    // "Which table am I sitting at?" spent all four calls on household, invitation, itinerary and
    // RSVP and never reached `get_my_table`, which is the one that answers the question. Sorting by
    // the same description overlap the generic branch uses puts the most on-topic one first. Ties
    // keep registry order, so nothing else about the plan changes.
    const personalCandidates = everything
      .filter((c) => /seat|table|rsvp|household|invitation|my_|benefit|voucher/.test(c.name) && !NEVER_AUTO.has(c.name) && (c.kind === 'read' || c.kind === 'draft'))
      .map((c, i) => ({ c, i, score: descriptionScore(c, q) }))
