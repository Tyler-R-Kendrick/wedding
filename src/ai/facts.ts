 ...
 * "not yet decided; TODO(Tyler & Sara)" and their hint text never reaches the model: a TODO is not
 * knowledge (ADR-0003 rule 6).
 */
export const UNDECIDED = 'not yet decided; TODO(Tyler & Sara)';
...
export function textLines(prefix: string, text: string, placeholder = false): string[] {
  ...
  if (dropped) out.push(facts.length ? `${prefix}other details ${UNDECIDED}.` : `${prefix}${UNDECIDED}.`);
  return out;
}

function scalarLines(key: string, value: unknown, depth: number): string[] {
  const prefix = `${label(key)}: `;
  if (isTextBlock(value)) return textLines(prefix, value.text, value.placeholder);
  if (typeof value === 'string') return value.trim() ? textLines(prefix, value) : [];
  ...
  if (Array.isArray(value)) {
    ...
    if (value.every(isTextBlock)) return value.flatMap((v) => textLines(prefix, v.text, v.placeholder));
    if (depth >= 2) return [];
...
export function factsFromOutcome(descriptor: Pick<AnyCapability, 'name' | 'title' | 'kind' | 'annotations'>, outcome: CapabilityOutcome<unknown>, opts: FactsOptions): SpotlightedSource[] {
  ...
  const fallbackCitation: Citation = first
    ? { ...first, url: first.url ?? homeRoute }
    : { sourceId: `capability:${descriptor.name}` as ContentSourceId, title: descriptor.title, url: homeRoute, verifiedAt: outcome.retrievedAt };
