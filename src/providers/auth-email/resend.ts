...
export class ResendAuthEmail implements AuthEmailProvider {
  ...
  async health() {
    try {
      ...
      return { status: 'down' as const, checkedAt: new Date().toISOString(), detail: e instanceof Error ? `${e.name}: ${e.message}; subjectIdLength=${this.userId.length}` : 'Connect unavailable' };
    }
  }
