 ...
 *
 * ## Why `guest_id` and `household_id` here are NOT foreign keys
 *
 * Levels 08, 09 and 10 each shipped an ownership column as bare `text` where a key belonged, and
 * the integration checklist now looks for exactly that. This schema is the deliberate exception,
 * and it is written down so the next level's checklist does not "fix" it:
 *
 *  - The consent ledger is APPEND-ONLY EVIDENCE. A cascade from `public.guests` would delete the
 *    record that a guest consented, at the moment the guest row goes — destroying the proof BIPA
 *    readiness exists to keep. `restrict`/`no action` would instead block guest deletion on vault
 *    contents, making an unrelated admin action fail with a cross-schema constraint error.
 *  - Guest deletion is already handled, deliberately, by the vault's OWN job: `DELETION_REASONS`
 *    contains `guest_deleted`, and `biometric.deletions` records what was destroyed with a proof.
 *    A database cascade would do that work silently, leaving no deletion record — a destruction the
 *    system could not evidence, which is the same failure as not performing it.
 *  - ADR-0006 §2 keeps the two schemas uncoupled on purpose. Referential integrity within the
 *    vault is still worth having, so the intra-vault references below ARE real keys.
 *
 * `matches.asset_id` is left unkeyed for the same reason: it points into the public media schema,
 * and match rows are removed by the deletion job and the retention sweep, not by a cascade whose
 * effects the vault could not record.
 */
...
export const biometricIdentityRefs = biometricSchema.table(
  'identity_refs',
  {
    id: text('id').primaryKey(),
    guestId: text('guest_id').notNull(),
    /** Intra-vault, so a real key: an enrolment that names no consent is exactly what must not exist.
        No cascade — consents are never deleted, and a dangling ref should surface, not vanish. */
    consentId: text('consent_id')
      .notNull()
      .references(() => biometricConsents.id),
...
export const biometricMatches = biometricSchema.table(
  'matches',
  {
    id: text('id').primaryKey(),
    guestId: text('guest_id').notNull(),
    /** Intra-vault, cascading: deleting the enrolled reference must take its match results with it,
        which is what a revocation or a deletion request means. */
    identityRefId: text('identity_ref_id')
      .notNull()
      .references(() => biometricIdentityRefs.id, { onDelete: 'cascade' }),
