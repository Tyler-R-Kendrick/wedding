...
export const mediaAiAnnotations = pgTable(
  'media_ai_annotations',
  {
    ...
    /**
     * A real key, cascading. An annotation is derived entirely from one asset and means nothing
     * without it, so a deleted asset must not leave its suggested caption, tags and index text
     * behind — the same defect (an ownership column as bare `text`) that levels 08, 09 and 10 each
     * shipped and each had caught by the constraint rather than by a test.
     */
...
export const mediaAiClusters = pgTable(
  'media_ai_clusters',
  {
    ...
    /**
     * Deliberately NOT a key, and not a column of keys: a cluster is a recomputed grouping, its
     * membership is a JSON array, and the `media.cluster` job rebuilds it from whatever assets
     * still exist. `representativeAssetId` cascades because a cluster whose representative is gone
     * has nothing to show for itself; the job then re-forms the group on its next run.
     */
    assetIds: jsonb('asset_ids').$type<string[]>().notNull(),
    representativeAssetId: text('representative_asset_id')
      .notNull()
      .references(() => mediaAssets.id, { onDelete: 'cascade' }),
