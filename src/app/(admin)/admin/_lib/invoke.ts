...
export async function adminInvoke<T = unknown>(name: string, input: unknown, opts: { method?: 'GET' | 'POST' } = {}): Promise<Result<CapabilityOutcome<T>, CapabilityError> & { principal: Principal }> {
  ...
  Object.assign(ctx.services, { requestHeaders: h, clientIp: getClientIp(h), cookieSink: { setCookies: [] } });
