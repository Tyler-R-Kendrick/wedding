...
export async function deleteGuest(fd: FormData): Promise<void> {
  finish('/admin/guests', await adminInvoke('admin_delete_guest', { guestId: str(fd, 'guestId') }), 'Guest deleted.');
}

export async function mergeGuests(fd: FormData): Promise<void> {
  finish('/admin/guests', await adminInvoke('admin_merge_guests', { keepId: str(fd, 'keepId'), mergeId: str(fd, 'mergeId') }), 'Guests merged.');
}

export async function resetIdentity(fd: FormData): Promise<void> {
  finish('/admin/guests', await adminInvoke('admin_reset_identity', { guestId: str(fd, 'guestId'), reason: str(fd, 'reason') || 'admin reset' }), 'Access reset; the guest can claim again.');
}

export async function rebindIdentity(fd: FormData): Promise<void> {
  finish('/admin/guests', await adminInvoke('admin_rebind_identity', { guestId: str(fd, 'guestId'), email: str(fd, 'email'), reason: str(fd, 'reason') || 'admin rebind' }), 'Access moved to the new email.');
}

export async function importGuestsCsv(fd: FormData): Promise<void> {
  const r = await adminInvoke<{ dryRun: boolean; householdsCreated: number; guestsCreated: number; guestsUpdated: number; skipped: number; issues: { line: number; message: string }[] }>('admin_import_guests_csv', { csv: str(fd, 'csv'), dryRun: bool(fd, 'dryRun') });
...
export async function deleteHousehold(fd: FormData): Promise<void> {
  finish('/admin/households', await adminInvoke('admin_delete_household', { householdId: str(fd, 'householdId') }), 'Household deleted.');
}

export async function revokeInvitation(fd: FormData): Promise<void> {
  finish('/admin/invitations', await adminInvoke('admin_revoke_invitation', { invitationId: str(fd, 'invitationId'), reason: str(fd, 'reason') || 'revoked by admin' }), 'Invitation link revoked.');
}
...
export async function issueInvitation(_prev: IssuedLink, fd: FormData): Promise<IssuedLink> {
  ...
  const r = rotate
    ? await adminInvoke<{ url: string; qrSvg: string; invitation: { householdId: string; expiresAt: string } }>('admin_rotate_invitation', { invitationId: rotate })
...
export async function setAdminRole(fd: FormData): Promise<void> {
  ...
  finish('/admin/guests', await adminInvoke('admin_set_admin_role', { email: str(fd, 'email'), role: role === 'none' ? null : role }), 'Admin role updated.');
}
